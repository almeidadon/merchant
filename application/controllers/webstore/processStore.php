...
/**
 * This file builds the version 1.0 of the Buttn API used for merchant integration.
...
class ProcessStore extends CI_Controller
{
    ...
    function transactions() //Function validates the incoming requests
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'GET' || $this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $data['app_used']            = 'WEBSTORE'; //Hard code in each API
            ...
            } else if (isset($data['checksum']) AND $data['checksum'] != 0) {
                if (!$this->paymentgateway->isValidChecksum($data)) {
