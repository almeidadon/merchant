...
class Tank_auth
{
	...
	function login_by_otp($login, $password)
	{
		$remember = '1'; //autologin the user
        $login_by_username = '1';
        $login_by_email = '1';
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				$data['mobileNo'] = $login ;
				$data['otp'] = $password ;
				if ($this->ci->notification_lib->validateOtp($data)) {		// password ok
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $name_of_customer,$registration_mode, $email_activation)
	{
	...
	{
		return $this->ci->users->get_total_wallet_balance($user_id);
	}
	
	function get_total_stored_cards($user_id)
	{
		return $this->ci->users->get_total_stored_cards($user_id);
	}
	
	function get_total_transaction_amount($user_id)
	{
		return $this->ci->users->get_total_transaction_amount($user_id);
	}
}
